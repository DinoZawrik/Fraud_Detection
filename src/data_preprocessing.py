...
def time_features(X):
    ...
    X_transformed['time_sin'] = np.sin(2 * np.pi * X_transformed['time_sec_day'] / seconds_in_day)
    X_transformed['time_cos'] = np.cos(2 * np.pi * X_transformed['time_sec_day'] / seconds_in_day)
...
def create_scaling_transformer():
    """Создает ColumnTransformer для масштабирования."""
